...
Cursorを使った開発を通じて、以下のような知見が得られました：

- メリット
  - 複雑な認証フローの実装でも、自然言語での指示だけで実装が可能
  - エラーが発生した際も、的確な修正案を提示してくれる
  - ドキュメントの参照や実装の方針決定が効率的
- デメリット
  - 細かい実装の調整には複数回のやり取りが必要
  - プロンプトの書き方によって生成されるコードの品質に差が出る
...
パスキーについて、Cursorを使ってアプリケーションを開発しながら紹介しました。今回の経験を通じて、Cursorが複雑な認証機能の実装をサポートしてくれることがわかりました。
